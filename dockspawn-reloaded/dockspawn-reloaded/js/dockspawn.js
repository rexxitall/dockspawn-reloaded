 ...
 * @return {number}
 */
var SORTZINDEX = function (diva, divb) {
    var ziA = parseFloat(diva.style.zIndex);
    var ziB = parseFloat(divb.style.zIndex);
    var result = 0;
    result=ziA - ziB;
    return result;
};
...
function reorderZIndex() {
    ...
    */
    divs = null;
    //floating items exept the wheel has either tab-handle or dialog-floating as class
    // the wheel classes start with "dock-wheel-base" or ""dock-wheel-item""
    divs = document.getElementsByClassName('tab-handle');
    var i=0;
    ...
    divs = null;
    divs = document.getElementsByClassName('dialog-floating');
    for ( i = 0; i < divs.length; i++) {
        if (divs[i].style.zIndex > "") {
            ARR.push(divs[i]);
        }
    ...
    for ( i = 0; i < ARR.length; i++) {
        if (ARR[i].style.zIndex !== oldZI) {
            ZInew++;
        }
        oldZI = ARR[i].style.zIndex;
        ARR[i].style.zIndex = ZInew;
    }
...
(function () {
    ...
    dockspawn.TabHandle = function (parent) {
        ...
        this.zIndexCounter = ZI_BASE;
        console.log(this.zIndexCounter);
    ...
    dockspawn.TabHandle.prototype._bringToFront = function (element) {
        element.style.zIndex = ZI_MAX + 1;
        reorderZIndex();
        this.zIndexCounter = element.style.zIndex;
    };
...
//----------------------------------------------------------------------------------------
    ...
    dockspawn.TabPage.prototype.setSelected = function (flag) {
        ...
        console.log("tabpagesetselected");
        if (this.selected) {
...
//            if something becomes undocked it will mutate to a so called dialog
...
//----------------------------------------------------------------------------------------
    ...
    dockspawn.Dialog.fromElement = function (id, dockManager) {
        ...
        this.hight = ELM.offsetHeight;
        this.width = ELM.offsetWidth;
        return new dockspawn.Dialog(new dockspawn.PanelContainer(ELM, dockManager), dockManager);
    };

    dockspawn.Dialog.prototype._initialize = function () {
        console.log("float initialize");
...
//-----------------------------------------------------
        // We have to use the right base otherwise the Dialog will not be usable anymore (lost its event context and doc man will not know him anymore)
//-----------------------------------------------------
        console.log("runs in container" + this.dockManager.element.id);
        try {
            var divid = this.dockManager.element.id;
            document.getElementById(divid).appendChild(this.elementDialog);
            console.log("runs in container" + this.dockManager.element.id);
        }
        catch (e) {
            console.log("runs in body mode");
            document.body.appendChild(this.elementDialog);
        }


        //.RootNode.appendChild(this.elementDialog);
        //und wenn wir das element auch an den richtigen Root node klemmen, dann wirds auch was midde Docking....
        //---------------------------------------------------
    ...
    dockspawn.Dialog.prototype.onMouseDown = function (e) {
        ...
        console.log("mousedown bring to front");
        //console.log(e);
        this.bringToFront();
        console.log(this.elementDialog.style.zIndex);

        this.elementDialog.style.position = "absolute";
    };
    ...
    dockspawn.Dialog.prototype.bringToFront = function () {
        this.elementDialog.style.zIndex = ZI_MAX + 1;
        reorderZIndex(); //this will also now reorder all other elements nicely
    };

//----------------------------------------------------------------------------------------

//            D R A G A B L E   C O N T A I N E R

//used for mousemovements of dialogs.
...
//----------------------------------------------------------------------------------------
    ...
    dockspawn.DraggableContainer.prototype.onMouseMove = function (event) {
        //those movements have to be limited acc the docspawn container size to keep them visible

        try {
            var divid = this.dockManager.element.id;
            border = document.getElementById(divid);
        ...
        var x = event.pageX;
        var y = event.pageY;
        var maxX = parseInt(border.style.width);
        var maxY = parseInt(border.style.height);
/*
        var minX = parseInt(border.style.left);
        var minY = parseInt(border.style.top);
        var minX = parseInt(border.style.right);
        var minY = parseInt(border.style.bottom);
*/
...
//----------------------------------------------------------------------------------------
//            R E S I Z E A B L E   C O N T A I N E R
...
//      context.model.rootNode.detachFromParent();
    ...
    dockspawn.DockManager.prototype.requestUndockToDialog = function (container, event, dragOffset) {
        ...
        dialog.resize (dialog.elementDialog.clientWidth,dialog.elementDialog.clientHeight-1); //needed to adjust the shadow nicely...
        dialog.resize (dialog.elementDialog.clientWidth,dialog.elementDialog.clientHeight); //TODO : find the shadow routine and do it without this hack
        return dialog;
    };
...
//----------------------------------------------------------------------------------------
//            D O C K         L A Y O U T E N G I N E
...
//  ??? either he notiefies layout engine or the clients directly got less gras
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
    ...
    dockspawn.DockWheel = function (dockManager) {
        ...
        this.elementMainWheel.style.zIndex = zIndex;
        ...
        //---------------------------------------------------
        this.elementPanelPreview.style.zIndex = 0;

        //this.elementPanelPreview.style.backgroundColor="ff0000";
    ...
    dockspawn.DockWheel.prototype.showWheel = function () {
        if (this.dockManager.wheelDisabled)
            return;
        console.log("show wheel");
        this._visible = true;
        if (!this.activeNode) {
        ...
        this.elementMainWheel.style.zIndex = ZI_WHEEL + 1;
        this.elementSideWheel.style.zIndex = ZI_WHEEL + 1;
    ...
    dockspawn.DockWheel.prototype.hideWheel = function () {
        ...
        this.elementMainWheel.style.zIndex = ZI_BASE;
        this.elementSideWheel.style.zIndex = ZI_BASE;
        removeNode(this.elementMainWheel);
        removeNode(this.elementSideWheel);
...
//----------------------------------------------------------------------------------------
//            D O C K    F I L L  D O C K   C O N T A I N E R
//     Manages the dock overlay buttons that are displayed over the dock manager
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
//            S P L I T T E R   D O C   C O N T A I N E R
//----------------------------------------------------------------------------------------
...
//					H O R I Z O N T A L    D O C K    C O N T A  I N E R
...
//----------------------------------------------------------------------------------------
    ...
    dockspawn.PanelContainer = function (elementContent, dockManager, title) {
        ...
        this.savedMaximized = false;
        this.savedMinMaxWidth = 0;
        this.savedMinMaxHeight = 0;
        this.savedMinMaxCssText = "";

        this.savedRolled = false;
        this.savedRollerHeight = 0;
    ...
    dockspawn.PanelContainer.prototype._initialize = function () {
        ...
        // title of panel window
        console.log(this.elementTitle);
        this.elementPanel.appendChild(this.elementTitle);
        this.elementTitle.appendChild(this.elementTitleText);


        // action buttons for the dialog window close rullup and down min and maximize

        this.elementButtonClose = document.createElement('div');
        this.elementButtonMinMax = document.createElement('div');
        this.elementButtonRoller = document.createElement('div');

        //font awesome icons...
        this.elementButtonClose.innerHTML = '<i class="icon-remove"></i>';
        this.elementButtonClose.classList.add("panel-titlebar-button-close");

        this.elementButtonMinMax.innerHTML = '<i class="icon-plus"></i>';
        this.elementButtonMinMax.classList.add("panel-titlebar-button-minmax");

        this.elementButtonRoller.innerHTML = '<i class="icon-chevron-up"></i>';
        this.elementButtonRoller.classList.add("panel-titlebar-button-roller");

        // add the buttons to the title element
        this.elementTitle.appendChild(this.elementButtonClose);
        this.elementTitle.appendChild(this.elementButtonMinMax);
        this.elementTitle.appendChild(this.elementButtonRoller);
        ...
        //  add the window button event handler
        ...
        this.minmaxButtonClickedHandler = new dockspawn.EventHandler(this.elementButtonMinMax, 'click', this.onMinMaxButtonClicked.bind(this));
        this.rollerButtonClickedHandler = new dockspawn.EventHandler(this.elementButtonRoller, 'click', this.onRollerButtonClicked.bind(this));
        // disable wheel from beeing diplayed if those buttons are used
        this.closeButtonMouseOverHandler = new dockspawn.EventHandler(this.elementButtonClose, 'mouseover', this.onCloseButtonMouseOver.bind(this));
        this.minmaxButtonMouseOverHandler = new dockspawn.EventHandler(this.elementButtonMinMax, 'mouseover', this.onMinMaxButtonMouseOver.bind(this));
        this.rollerButtonMouseOverHandler = new dockspawn.EventHandler(this.elementButtonRoller, 'mouseover', this.onRollerButtonMouseOver.bind(this));

        this.closeButtonMouseOutHandler = new dockspawn.EventHandler(this.elementButtonClose, 'mouseout', this.onCloseButtonMouseOut.bind(this));
        this.minmaxButtonMouseOutHandler = new dockspawn.EventHandler(this.elementButtonMinMax, 'mouseout', this.onMinMaxButtonMouseOut.bind(this));
        this.rollerButtonMouseOutHandler = new dockspawn.EventHandler(this.elementButtonRoller, 'mouseout', this.onRollerButtonMouseOut.bind(this));
    ...
    dockspawn.PanelContainer.prototype.destroy = function () {
        ...
        if (this.minmaxButtonClickedHandler) {
            this.minmaxButtonClickedHandler.cancel();
            delete this.minmaxButtonClickedHandler;
        }

        if (this.rollerButtonClickedHandler) {
            this.rollerButtonClickedHandler.cancel();
            delete this.rollerButtonClickedHandler;
        }

        //---------------------------------------------
        if (this.closeButtonMouseOverHandler) {
            this.closeButtonMouseOverHandler.cancel();
            delete this.closeButtonMouseOverHandler;
        }

        if (this.minmaxButtonMouseOverHandler) {
            this.minmaxButtonMouseOverHandler.cancel();
            delete this.minmaxButtonMouseOverHandler;
        }

        if (this.rollerButtonMouseOverHandler) {
            this.rollerButtonMouseOverHandler.cancel();
            delete this.rollerButtonMouseOverHandler;
        }
        //---------------------------------------------
        if (this.closeButtonMouseOutHandler) {
            this.closeButtonMouseOutHandler.cancel();
            delete this.closeButtonMouseOutHandler;
        }

        if (this.minmaxButtonMouseOutHandler) {
            this.minmaxButtonMouseOutHandler.cancel();
            delete this.minmaxButtonMouseOutHandler;
        }

        if (this.rollerButtonMouseOutHandler) {
            this.rollerButtonMouseOutHandler.cancel();
            delete this.rollerButtonMouseOutHandler;
        }
...
// CLOSE BUTTON
    ...
    dockspawn.PanelContainer.prototype.onCloseButtonMouseOver = function (e) {
        this.dockManager.wheelDisabled = true
    };
    dockspawn.PanelContainer.prototype.onCloseButtonMouseOut = function (e) {
        this.dockManager.wheelDisabled = false
    };

// MIN MAX BUTTON
    dockspawn.PanelContainer.prototype.onMinMaxButtonClicked = function (e) {
        //   TODO : undock panels and atbs on button clicks
        //   if (!this.floatingDialog) {this.dockManager.performUndockToDialog(this);}


        if (this.floatingDialog) {

            //console.log(this.floatingDialog.elementDialog.getBoundingClientRect() );
            // contains complete floating Dialog incl. its shadow and so on

            if (this.savedMaximized) {
                this.savedMaximized = false;
                this.elementButtonMinMax.innerHTML = '<i class="icon-plus"></i>';
                // restore old size
                this.resize(this.savedMinMaxWidth, this.savedMinMaxHeight);
                // move Dialog to old position
                this.floatingDialog.elementDialog.style.cssText = this.savedMinMaxCssText;

            } else {

                this.savedMaximized = true;
                this.elementButtonMinMax.innerHTML = '<i class="icon-minus"></i>';

                //get the main DocManager container size
                var dockManRect = document.getElementById(this.dockManager.element.id).getBoundingClientRect();

                //get and save the current window position, elementDialog contains all also its shadow wrapper and so on
                dialogRect = this.floatingDialog.elementDialog.getBoundingClientRect();
                this.savedMinMaxCssText = this.floatingDialog.elementDialog.style.cssText;
                this.savedMinMaxHeight = dialogRect.height;
                this.savedMinMaxWidth = dialogRect.width;
                //perform size change
                this.resize(dockManRect.width, dockManRect.height);
                // move Dialog to upper left edge of the Main container
                this.floatingDialog.elementDialog.style.cssText = "position:absolute;left:" + dockManRect.left + "px;top:" + dockManRect.top + "px;"; // seems the only way to apply this changes
                this.floatingDialog.elementDialog.style.zIndex = ZI_MAX + 1;
                reorderZIndex();
    ...
    dockspawn.PanelContainer.prototype.onMinMaxButtonMouseOver = function (e) {
        this.dockManager.wheelDisabled = true;
    };
    dockspawn.PanelContainer.prototype.onMinMaxButtonMouseOut = function (e) {
        this.dockManager.wheelDisabled = false;
    };


// ROLL UP ROLL DOWN BUTTON
    dockspawn.PanelContainer.prototype.onRollerButtonClicked = function (e) {


        if (this.floatingDialog) {
            //this.floatingDialog.destroy()
            var titleBarHeight = this.elementTitle.clientHeight;
            if (this.elementTitle.clientHeight === this.height) {
                this.elementButtonRoller.innerHTML = '<i class="icon-chevron-up"></i>';
                this.resize(this.width, this.savedRollerHeight);
                this.savedRolled = false;
            } else {
                this.elementButtonRoller.innerHTML = '<i class="icon-chevron-down"></i>';
                this.savedRollerHeight = this.height;
                this.resize(this.width, titleBarHeight);
                this.savedRolled = true;
            }
        } else {
            // this.performUndock();
            // this.destroy()
    ...
    dockspawn.PanelContainer.prototype.onRollerButtonMouseOver = function (e) {
        this.dockManager.wheelDisabled = true
    };
    dockspawn.PanelContainer.prototype.onRollerButtonMouseOut = function (e) {
        this.dockManager.wheelDisabled = false
    };

//----------------------------------------------------------------------------------------

//					V E R T I C A  L    D O C K    C O N T A  I N E R
...
//----------------------------------------------------------------------------------------
//					D O C       G R A P H        D E S E R I A L I Z E R
